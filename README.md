...
1. Set `WC_DUPLICATE_ORDER_FIX` to `false` in docker-compose.yml
2. Restart: `docker-compose restart wordpress`
3. Add product to cart, fill checkout form
4. Click **Run Concurrent Test** → See duplicates
5. Set constant to `true`, restart, repeat → Only 1 order
...
```php
// wp-config.php or docker-compose.yml
...
define( 'WC_CONCURRENT_CHECKOUT_REQUESTS', 5 );   // Concurrent requests to send
```
