...
class Prevent_Duplicate_Orders {
	...
	public static function acquire_lock() {
		...
		// After acquiring lock, check if order was JUST created with this cart hash.
		// 60-second window catches concurrent requests but allows legitimate repeat orders.
		$existing_order = self::find_recent_order( $cart_hash, $email, 60 );
		if ( $existing_order ) {
			self::release_lock();
			throw new Exception(
				sprintf(
	...
	/**
	 * Find an order created within the time window.
	 *
	 * @since 1.0.0
	 *
	 * @param string $cart_hash      Cart hash to search for.
	 * @param string $email          Customer email.
	 * @param int    $seconds_window Time window in seconds.
	 *
	 * @return int|null Order ID if found, null otherwise.
	 */
	private static function find_recent_order( $cart_hash, $email, $seconds_window ) {
		global $wpdb;
		...
		$order_id = $wpdb->get_var(
			$wpdb->prepare(
				...
				$cart_hash,
				$email,
				gmdate( 'Y-m-d H:i:s', time() - $seconds_window )
			)
		);
